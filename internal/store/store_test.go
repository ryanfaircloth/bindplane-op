...
func runCaseInsensitiveCasePreservingTests(t *testing.T, store Store) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	testSource := func(name string) model.Resource {
		return model.NewSource(name, "macos", []model.Parameter{})
	}
		...
		t.Run(test.name, func(t *testing.T) {
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					...
					store.Clear()
					applyTestTypes(t, store)
					_, err := store.ApplyResources(ctx, []model.Resource{testSource(test.initialName)})
					require.NoError(t, err, "expect no error in seed apply")

					statuses, err := store.ApplyResources(ctx, []model.Resource{testSource(test.addName)})
					require.NoError(t, err, "expect no error on valid apply call")

					// expect configured (name-case change) with new name
					assert.Equal(t, model.StatusConfigured, statuses[0].Status)
					assert.Equal(t, test.addName, statuses[0].Resource.Name())

					// still accessible with old name
					source0, err := store.Source(ctx, test.initialName)
					require.NoError(t, err)
					require.Equal(t, test.addName, source0.Name())

					// accessible with new name
					source1, err := store.Source(ctx, test.addName)
					require.NoError(t, err)
					require.Equal(t, test.addName, source1.Name())

					// same result either way
					require.Equal(t, source0, source1)
				})
			}
		})
	}
